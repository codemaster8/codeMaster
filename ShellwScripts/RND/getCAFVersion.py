...
for key in sorted(clusterName):
        ...
        cluster_value = subprocess.Popen('sshpass -p "Vertis@28" ssh -o StrictHostKeyChecking=no -t okhatav@{0} unzip -c /opt/jetty_caf/jetty/wars/admin.war META-INF/MANIFEST.MF | grep Implementation-Version'.format(machineName), stdout=subprocess.PIPE, stderr=None, shell=True)
