...
for index in range(len(clusterName)):
        ...
        cluster_value = subprocess.call('sshpass -p "Vertis@28" ssh -o StrictHostKeyChecking=no -t okhatav@{0} grep clu1 /opt/jetty_coordinator/jetty/conf/dece.properties'.format(machineName), shell=True)
        if cluster_value == 0:
