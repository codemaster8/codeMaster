...
for key in sorted(clusterName):
        ...
        cluster_value = subprocess.Popen('sshpass -p "Vertis@28" ssh -o StrictHostKeyChecking=no -t okhatav@{0} grep clu /opt/jetty_email_notifier/jetty/conf/email.properties'.format(machineName), stdout=subprocess.PIPE, stderr=None, shell=True)
