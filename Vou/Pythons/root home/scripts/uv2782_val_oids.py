...
def verify_accounts(conn):
	...
	sql="select account_oid, display_name, status from account where account_oid in %s and (DISPLAY_NAME<>'@@@deleted@@@' or status<>'deidentified')" % (acc_list)
	db_curs.execute(sql)
	recs=db_curs.fetchall()
	if len(recs)>0:
		print "Following accounts have NOT been properly deidentified"
		print "AccountOID, DisplayName, Status"
		for rec in recs:
...
def verify_users(conn):
	...
	sql="select user_oid, username, status from account_user where user_oid in " + user_list + " and (username not like '@@@deleted@@@%' or status<>'deidentified')"
	db_curs.execute(sql)
	recs=db_curs.fetchall()
	if len(recs)>0:
...
def verify_acc_history(conn):
	print banner
	print "VERIFYING NEW ENTRIES in ACCOUNT_STATUS_HISTORY"
	print banner
	global account_oids
	db_curs=conn.cursor()
	print "Following account OIDs have not been inserted into account_status_history or have the incorrect status"
	print "Accound_Oid"
	for acc_oid in account_oids:
		sql="select account_oid, status from account_status_history where account_oid='%s' and created_date>sysdate-1/24 and status<>'deidentified'" % (acc_oid)
		db_curs.execute(sql)
		recs=db_curs.fetchall()
		if len(recs)==0:
			print acc_oid
...
def verify_user_history(conn):
	print banner
	print "VERIFYING NEW ENTRIES in USER_STATUS_HISTORY"
	print banner
	global user_oids
	db_curs=conn.cursor()
	print "Following user OIDs have not been inserted into user_status_history or have the incorrect status"
	print "User_Oid"
	for user_oid in user_oids:
		sql="select user_oid, status from user_status_history where user_oid='%s' and created_date>sysdate-1/24 and status<>'deidentified'" % (user_oid)
		db_curs.execute(sql)
		recs=db_curs.fetchall()
		if len(recs)==0:
			print user_oid
...
if __name__=="__main__":
	...
	verify_acc_history(conn)
	verify_user_history(conn)
	print "Done !"
