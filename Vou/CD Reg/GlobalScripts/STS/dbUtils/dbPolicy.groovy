...
class dbPolicy extends dbUtils 
{
	...
	*/
	...
	def getLatestTOUConsentVersionForAGeography(String consentCountry) 
    {   
		String rData = "";
		this.sql = "select CONSENT_VERSION from  CONSENT where CONSENT_URN = 'urn:dece:type:policy:TermsOfUse' and COUNTRY = '" + consentCountry + "'";	
		try {
            def rowData = dbConnExecuteQuery(this.sql);
            ...
            if(!rowData.isEmpty())
                {              
                    if (rowData.size == 1) 
                    {
						rData = rowData[0].CONSENT_VERSION;
						if (this.log) 
						{
							this.log.info "getLatestTOUConsentVersionForAGeography: sql is " + this.sql;
							this.log.info "getLatestTOUConsentVersionForAGeography: rowData.size is " + rowData.size;
							this.log.info "getLatestTOUConsentVersionForAGeography: rowData[0] is " + rowData[0];
						}                                              
                    } 
                } 
            }
