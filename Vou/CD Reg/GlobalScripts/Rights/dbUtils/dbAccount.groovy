...
class dbAccount extends dbUtils
{
	...
	*/
	...
	private dbSetStatusSQL_NativeDRM(String DRM, String status) {
		this.sql = "update DEVICE set STATUS = '%%STATUS%%' where DEVICE_OID = '%%DRM%%'";
		this.sql = sql.replaceAll("%%STATUS%%", status);
		this.sql = sql.replaceAll("%%DRM%%", DRM);
	}
	/*
	*  GET Methods
	*/
	...
	*/
	def accountGetRow(String oid) {
		...
		"NEWSFEED_CLEAR_TIME, " +
		"COUNTRY," +
		"DECE_DOMAIN_ID from ACCOUNT where ACCOUNT_OID = '$oid'";
	...
	*/
	...
	def NativeDRMSetStatus(String oid, String status) {
		def rc = 0;

		dbSetStatusSQL_NativeDRM(oid,status);
		...
		{	if(!rc.toString().isEmpty())
			{
				if (this.log) 
				{
					this.log.info "NativeDRMSetStatus: rc is "  + rc;
					this.log.info "NativeDRMSetStatus: sql is " + this.sql;
				}
			}
		}
		catch(Throwable e)
