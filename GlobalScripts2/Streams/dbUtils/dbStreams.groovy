...
class dbStreams extends dbUtils 
{
	...
	*/
	def streamGetRow(String oid) {
		...
		"from STREAM_DATA where STREAM_HANDLE_OID  = hextoraw('" + oid + "')";
	...
	*/
	def streamGetCreatedDate(String streamHandle) { 
		...
		this.sql = "select CREATED_DATE from STREAM_DATA where " + 
			"STREAM_HANDLE_OID = hextoraw('" + streamHandle + "')"; 
		try
		{	
	...
	*/
	def streamGetExpireDate(String streamHandle) { 
		...
		this.sql = "SELECT TO_CHAR(EXPIRATION_DATE, 'YYYY/MM/DD HH24:MI:SS') AS TIME " +
			"FROM STREAM_DATA where STREAM_HANDLE_OID = hextoraw('" + streamHandle + "')";
	...
	*/
	def streamHandleVerifyExpiration(String streamHandle) { 
		...
		this.sql = "SELECT EXTRACT (DAY FROM (expiration_date - created_date))*24+ " + 
	...
	*/
	def streamDiffCDateandTOD(String streamHandle) { 
		...
		this.sql = "SELECT (extract(DAY FROM sysdate - CREATED_DATE)*24*60*60)+ " + 
			...
			"as sec FROM STREAM_DATA where STREAM_HANDLE_OID = hextoraw('" + streamHandle + "')";
	...
	*/
	def streamIsActive(String streamHandle) { 
		...
		this.sql = "select count(*) as CNT from STREAM_DATA where " + 
			"STREAM_HANDLE_OID = hextoraw('" + streamHandle + "') and STATUS = 'active'"; 
	...
	*/
	def streamIsDeleted(String streamHandle) { 
		...
		this.sql = "select count(*) as CNT from STREAM_DATA where " + 
			"STREAM_HANDLE_OID = hextoraw('" + streamHandle + "') and STATUS = 'deleted'"; 
	...
	*/
	def streamGetStreamDataHistoryRowCount(String accountoid) { 
		...
		this.sql = "select count(*) as CNT from STREAM_DATA_HISTORY where STREAM_HANDLE_OID in (select STREAM_HANDLE_OID from STREAM_DATA where ACCOUNT_OID = hextoraw('" + accountoid + "'))";
	...
	*/
	def streamGetStreamDataRowCount(String accountoid) { 
		...
		this.sql = "select count(*) as CNT from STREAM_DATA where STREAM_HANDLE_OID in (select STREAM_HANDLE_OID from STREAM_DATA where ACCOUNT_OID = hextoraw('" + accountoid + "'))";
	...
	*/
	def streamExpireDateEqualTOD(String streamHandle) { 
		...
		String Qsql = "SELECT TO_CHAR(EXPIRATION_DATE, 'YYYY/MM/DD HH24') AS TIME " +
			", TO_CHAR(sysdate - 2/24,'YYYY/MM/DD HH24') as TOD " +
			"FROM STREAM_DATA where STREAM_HANDLE_OID = hextoraw('" + streamHandle + "')";
	...
	*/
	def streamSetExpireTime(String streamHandle, String unit, float value) { 
		...
		if (value > 0) {
			this.sql = "UPDATE STREAM_DATA set EXPIRATION_DATE = sysdate + " + buf + " where STREAM_HANDLE_OID = hextoraw('" + streamHandle + "')";
		} else {
			this.sql = "UPDATE STREAM_DATA set EXPIRATION_DATE = sysdate " + buf  + " where STREAM_HANDLE_OID = hextoraw('" + streamHandle + "')";
	...
	*/
	def streamSetStatusDeleted(String streamHandle) { 
		...
		this.sql = "update STREAM_DATA set STATUS = 'deleted' where STREAM_HANDLE_OID = hextoraw('" + streamHandle + "')";
	...
	*/
	def streamSetStatusActive(String streamHandle) { 
		...
		this.sql = "update STREAM_DATA set STATUS = 'active',EXPIRATION_DATE = sysdate + 1 where STREAM_HANDLE_OID = hextoraw('" + streamHandle + "')";
	...
	*/
	def streamSetAccountOID(String streamHandle,String oid) { 
		...
		*/
		...
		this.sql = "update STREAM_DATA set ACCOUNT_OID = '" + oid + "' where STREAM_HANDLE_OID = hextoraw('" + streamHandle + "')"; 
	...
	*/
	def streamSetCreatedBy(String streamHandle,String nodeid) { 
		...
		this.sql = "update STREAM_DATA set CREATED_BY = '" + nodeid + "' where STREAM_HANDLE_OID = hextoraw('" + streamHandle + "')"; 
	...
	*/
	def streamDeleteHandle(String streamHandle) { 
		...
		this.sql = "delete from STREAM_DATA_HISTORY where STREAM_HANDLE_OID = hextoraw('" + streamHandle + "')";
		...
		{	
			...
			if(rc != 0) 
			{
				...
				*/
				this.sql = "delete from STREAM_DATA where STREAM_HANDLE_OID = hextoraw('" + streamHandle + "')";
	...
	*/
	def streamDeleteAccountOid(String oid) { 
		...
		this.sql = "delete from STREAM_DATA_HISTORY where STREAM_HANDLE_OID in (select STREAM_HANDLE_OID from STREAM_DATA where ACCOUNT_OID = hextoraw( '" + oid + "'))";
		...
		{	
			...
			if(rc != 0) 
			{
				...
				this.sql = "delete from STREAM_DATA where STREAM_HANDLE_OID in (select STREAM_HANDLE_OID from STREAM_DATA where ACCOUNT_OID = hextoraw('" + oid + "'))";
	...
	*/
	def streamStatusActive(String streamHandle) { 
		this.sql = "select STATUS from STREAM_DATA where " + 
			"STREAM_HANDLE_OID = hextoraw('" + streamHandle + "') and " +
			"STATUS = 'active'";
		try
		{	
			rc = this.dbConnExecuteUpdate(this.sql);
	...
	*/
	def streamSetCreatedDate(String streamHandle,int days) { 
		...
		this.sql = "update STREAM_DATA set CREATED_DATE = " + 
		   "sysdate $b $days where STREAM_HANDLE_OID = hextoraw('" + streamHandle + "')"; 
