...
class dbRightsToken extends dbUtils 
{
	...
	*/
	private dbSetStatusExpireSQL(String rightstoken, String timeStr) {
		this.sql = " update RIGHTS_TOKEN set RENTAL_ABSOLUTE_EXPIRATION = %%TIME%% " + 
			"where RIGHTS_TOKEN_OID = hextoraw('%%RT%%')";	
		this.sql = sql.replaceAll("%%RT%%",   rightstoken);
	...
	*/
	def rightstokenGetByApid(String apid, String accountoid) {
		...
		"and " +
		"RIGHTS_TOKEN.account_oid = hextoraw('" + accountoid + "')";
	...
	*/
	...
	/**
	* Returns the PURCHASE_PROFILE and PURCHASED_MEDIA_PROFILE rows of the RIGHTS_TOKEN_OID 
	* supplied in XML format.
	*   @version 1.0
	*
	* @param oid The RIGHTS_TOKEN_OID of the row to extract. 
	...
	*/
	def rightstokenPMPPGetRow(String oid) {
		...
		this.sql = "select " +
		"to_char(rawtohex(PURCHASED_MEDIA_PROFILE.PURCHASED_MEDIA_PROFILE_OID)) PURCHASED_MEDIA_PROFILE_OID, " +
		"to_char(rawtohex(PURCHASED_MEDIA_PROFILE.DISCRETE_MEDIA_FULFILLMENT)) DISCRETE_MEDIA_FULFILLMENT, " +
		"to_char(rawtohex(PURCHASE_PROFILE.PURCHASE_PROFILE_OID)) PURCHASE_PROFILE_OID, " +
		"to_char(rawtohex(PURCHASE_PROFILE.RIGHTS_TOKEN_OID)) RIGHTS_TOKEN_OID, " +
		"PURCHASE_PROFILE.DOWNLOAD, " +
		"PURCHASE_PROFILE.STREAM, " +
		"PURCHASE_PROFILE.DISCRETE_MEDIA_RIGHTS_REMAIN, " +
		"PURCHASE_PROFILE.PURCHASED_CONTENT_PROFILE, " +
		"PURCHASE_PROFILE.CREATED_DATE, " +
		"PURCHASE_PROFILE.UPDATED_DATE " +	
		"from " +
		"PURCHASED_MEDIA_PROFILE, PURCHASE_PROFILE where " +
		"PURCHASED_MEDIA_PROFILE.PURCHASE_PROFILE_OID = PURCHASE_PROFILE.PURCHASE_PROFILE_OID " +
		"and " +
		"PURCHASE_PROFILE.RIGHTS_TOKEN_OID = hextoraw('" + oid + "')";
...
/**
	...
	*/
	def rightsTokenDeleteByHandle(String rightstoken) {
		...
		this.sql = "select to_char(rawtohex(PURCHASED_MEDIA_PROFILE_OID)) PURCHASED_MEDIA_PROFILE_OID from PURCHASED_MEDIA_PROFILE " +
			"where PURCHASE_PROFILE_OID IN (select PURCHASE_PROFILE_OID from " +
			"PURCHASE_PROFILE where RIGHTS_TOKEN_OID = hextoraw('" + rightstoken + "'))"
		...
		if (!rowDataXML.isEmpty()) {
			...
			for (oid in oids.PURCHASED_MEDIA_PROFILE_OID) {
				this.log.info "oid is " + oid + "";
				this.sql = "DELETE PURCHASED_MEDIA_PROFILE where " +
				"PURCHASED_MEDIA_PROFILE_OID = '" + oid + "'";
				try
				{	
					rc = this.dbConnExecuteUpdate(this.sql);
		...
		*  PURCHASE_PROFILE
		*/
		this.sql = "delete PURCHASE_PROFILE where RIGHTS_TOKEN_OID = hextoraw('" + rightstoken + "')";
		...
		*  RIGHTS_SOLD_AS
		*/
		this.sql = "delete RIGHTS_SOLD_AS where RIGHTS_TOKEN_OID = hextoraw('" + rightstoken + "')";
		try
		{	
			rc = this.dbConnExecuteUpdate(this.sql);
		...
		*  FULFILLMENT_LOC
		*/
		this.sql = "delete FULFILLMENT_LOC where RIGHTS_TOKEN_OID = hextoraw('" + rightstoken + "')";
		try
		{	
			rc = this.dbConnExecuteUpdate(this.sql);
		...
		*/
		this.sql = "delete RIGHTS_STATUS_HISTORY where RIGHTS_TOKEN_OID = hextoraw('" + rightstoken + "')";
		try
		{	
			rc = this.dbConnExecuteUpdate(this.sql);
		...
		*  RIGHTS_TOKEN
		*/
		this.sql = "delete RIGHTS_TOKEN where RIGHTS_TOKEN_OID = hextoraw('" + rightstoken + "')";
		try
		{	
			rc = this.dbConnExecuteUpdate(this.sql);
	...
	*/
	def rightsTokenDeleteByAccountHandle(String accountoid) {
		...
		this.sql = "select to_char(rawtohex(RIGHTS_TOKEN_OID)) RIGHTS_TOKEN_OID from RIGHTS_TOKEN " +
			"where ACCOUNT_OID = hextoraw('" + accountoid + "') ";
