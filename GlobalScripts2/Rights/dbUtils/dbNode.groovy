	...
	*/
	private dbSetStatusSQL(String nodeId, String status) {
		if (nodeId =~ /urn:dece:org:org:dece:/) {
			this.sql = "update NODE set NODE_STATUS = '%%STATUS%%' where NODE_ID = +'urn:dece:org:org:dece:%%NODEOID%%'";
	...
	*/
	def nodeGetStatus(String oid) {
		...
		*/
		...
		this.sql = "SELECT NODE_STATUS FROM NODE WHERE NODE_ID = 'urn:dece:org:org:dece:" + oid + "'";
