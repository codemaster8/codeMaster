...
class dbPolicy extends dbUtils 
{
	...
	*/
	private sqlPolicyStatus(String oid, String status) {
		//this.sql = "update POLICY set STATUS = '" + status + "' where POLICY_OID = " + oid + ""; 
	...
	*
	*/
	def policyDelete(Integer policyClass, String userName, String policyStatus) 
	{
		private sqlStr = "";
		
		/*sqlStr = "update policy	 set status = 'deleted'	 where  created_by =  hextoraw('"+policyActor+"') and status = 'active' and policy_class = "+policyClass */
		sqlStr = "update policy set status = 'deleted' where status = 'active' and policy_oid in (select policy_oid from policy_relationship where policy_actor in ((select user_oid from account_user where lower(username) = lower('"+userName+"')), (select account_oid from account_user where lower(username) = lower('"+userName+"')))) and policy_class="+policyClass
