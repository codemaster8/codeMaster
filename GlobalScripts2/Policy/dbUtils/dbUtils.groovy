...
public class dbUtils
{
	...
	def String[] ReadDBCrecentials(String envIdentifier)
	{
		...
		switch (envIdentifier) 
		{
			...
			case "LP1":
				...
				break;
			case "LP2":
				dbUsername = "deqa2_app";
				dbPassword = "lp2db_23";
				break;
			case "LP3CH":
				log.info "########### Connecting to LP3 Charelotte";
				dbUsername = "deadm_app";
				dbPassword = "lp3db_23";
				break;
			case "LP3ST":
			log.info "########### Connecting to LP3 Sterling";
				dbUsername = "deadm_app";
                dbPassword = "lp3db_23";
				break;
			case "LP1-COPY":
	...
	{
		this.log.info("Inside dbConnEstablish method!!!!");	
		this.log.info("Parameter passed envIdentifier : " + envIdentifier);
		this.log.info("Get DB credential by calling  method ReadDBCrecentials ");
		private dbCredentials = ReadDBCrecentials(envIdentifier);
		dbUsername = dbCredentials[0];
		dbPassword = dbCredentials[1];	
		this.log.info("Credential returned User : " + dbUsername + ", password : " + dbPassword);
		private db;
		...
		else if (envIdentifier == "LP3ST")
                                {
									this.log.info("DB Env : '" + "LP3ST"+ "'");
                                try 
                                {
                                                db = [url: 'jdbc:oracle:thin:@stdeqp-clu2-scan.va.neustar.com:2115/stdecqlp.neustar.com', user: dbUsername, password: dbPassword, driver: 'oracle.jdbc.driver.OracleDriver'];
                                                this.dbConn = Sql.newInstance(db.url, db.user, db.password, db.driver);
						this.log.info("DB User : '" + db.user+ "'");
						this.log.info("DB Password : '" + db.password+ "'");
						this.log.info("DB url : '" + db.url + "'");						
						
                                }
                                catch(Throwable e) 
                                {
                                                if(this.log) 
                                                {		this.log.info("Error while connecting to DB!!!");
                                                                this.log.info e;   
                                                }
                                }
                                }              
		else if (envIdentifier == "LP3CH")
                                {
					this.log.info("DB Env : '" + "LP3CH"+ "'");

                                try 
                                {
                                                db = [url: 'jdbc:oracle:thin:@chdeqp-clu2-scan.nc.neustar.com:2115/chdecqlp.neustar.com', user: dbUsername, password: dbPassword, driver: 'oracle.jdbc.driver.OracleDriver'];
                                                this.dbConn = Sql.newInstance(db.url, db.user, db.password, db.driver);
						 this.log.info("DB User : '" + db.user+ "'");
						this.log.info("DB Password : '" + db.password+ "'");
						this.log.info("DB url : '" + db.url + "'");
		...
		else if (envIdentifier == "DEV_LP2_DEVDECESR5")
                                {
					this.log.info("DB Env : '" + "DEV"+ "'");
		...
		else if(envIdentifier == "LP1")
		{
		    try 
    		{
    			db = [url: 'jdbc:oracle:thin:@stdeqp-clu2-scan.va.neustar.com:2115/stdecqa.neustar.com', user: dbUsername, password: dbPassword, driver: 'oracle.jdbc.driver.OracleDriver'];
    			this.dbConn = Sql.newInstance(db.url, db.user, db.password, db.driver);
				log.info "Env: " + envIdentifier + "dbUserName:"+dbUsername;
    		}
    		catch(Throwable e) 
    		{
    			if(this.log) 
		...
		else if(envIdentifier == "LP2")
		{
		    try 
    		{
    			db = [url: 'jdbc:oracle:thin:@stdeqp-clu1-scan.va.neustar.com:2115/stdecqpdb_stdecqa.neustar.com', user: dbUsername, password: dbPassword, driver: 'oracle.jdbc.driver.OracleDriver'];
    			this.dbConn = Sql.newInstance(db.url, db.user, db.password, db.driver);
				log.info "Env: " + envIdentifier + "dbUserName:"+dbUsername;
    		}
    		catch(Throwable e) 
	...
	this.log.info("Out dbConnEstablish method!!!!");	
	}
	/**
	...
	{
		this.log.info("Inside dbUtils groovy method selectFromDB");
		...
		this.log.info("Out dbUtils groovy method selectFromDB");
   ...
   {
		this.log.info("Inside selectFromDBArray");
		this.log.info("Parameter passed  sqlStr : " + sqlStr + ", and envIdentifier : " +envIdentifier);
	   if(envIdentifier != null)
	   {
			this.log.info("Establish DB Connection for  environment : " + envIdentifier);
		   dbConnEstablish(envIdentifier);
	   }
	   else
	   {
		   this.log.info("Environment identifier is null");
		   if(dbConn == null)
		   {
			this.log.info("Establish DB Connection for  environment : " + this.dbIdentifier);
			dbConnEstablish(this.dbIdentifier);
		   }
	   }
	   private sqlResult;	   
	   ...
	   if(columnList.size() > 1)
	   {
			this.log.info("Out selectFromDBArray and returning 2D array : " + strArray2D);
		   return(strArray2D);
	   }
	   else
	   {
			this.log.info("Out selectFromDBArray and returning 1D array : " + strArray1D);
		   return(strArray1D);
	   }	   
   }
	/**
