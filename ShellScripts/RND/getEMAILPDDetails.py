...
for key in sorted(clusterName):
        ...
        cluster_value = subprocess.Popen('sshpass -p "Vertis@28" ssh -o StrictHostKeyChecking=no -t okhatav@{0} ps -ef | grep "jetty_email.state" | grep -v grep'.format(machineName), stdout=subprocess.PIPE, stderr=None, shell=True)
