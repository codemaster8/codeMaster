...
----------------------------------------------------
 ...
 ---
  ...
  7/27/11: Added methods userSetLoginFailures() & userGetLoginFailures() pjg.
  8/12/11: Added method userSetCreatedByNode() pjg
  8/19/11: Added method userSetCountry() pjg
  9/6/11:  Added method userGetCreatedByNode() pjg
  9/30/11: Added method userGetPassword() pjg
----------------------------------------------------
*/
...
class dbUser extends dbUtils 
{
	...
	*/
	def userGetRow(String oid) {
			...
			"from ACCOUNT_USER where USER_OID = hextoraw('" + oid + "')";
	...
	*/
	...
	/**
	* Extract the ACCOUNT_USER.PASSWORD associated with the USER_OID 
	* supplied. 
	* @version 1.0
	*
	* @param oid The ACCOUNT_USER.USER_OID to extract. 
	...
	*
	*/
	def userGetPassword(String oid) {
		String status = "";

		this.sql = "select PASSWORD from ACCOUNT_USER where " +
			"USER_OID = hextoraw('" + oid + "')"; 
		...
		{	
			...
			{	
				if (rowData.size == 1) 
				{
					status = rowData[0].PASSWORD;
					if (this.log) 
					{
						this.log.info "userGetPassword: sql is " + this.sql;
						this.log.info "userGetPassword: rowData.size is " + rowData.size;
						this.log.info "userGetPassword: rowData[0] is " + rowData[0];
						this.log.info "userGetPassword: status is " + status; 
					}			
				}
			}		 
		}
	...
	/**
	* Extract the ACCOUNT_USER.CREATED_BY_NODE_OID associated with the USER_OID 
	* supplied. 
	* @version 1.0
	*
	* @param oid The ACCOUNT_USER.USER_OID to extract. 
	*
	* @return ACCOUNT_USER.CREATED_BY_NODE_OID value 
	*
	*/
	def userGetCreatedByNode(String oid) {
		String createdby = "";

		this.sql = "select to_char(rawtohex(CREATED_BY_NODE_OID)) CREATED_BY_NODE_OID from ACCOUNT_USER where " +
			"USER_OID = hextoraw('" + oid + "')"; 
		...
		{	
			...
			{	
				if (rowData.size == 1) 
				{
					createdby = rowData[0].CREATED_BY_NODE_OID;
					if (this.log) 
					{
						this.log.info "userGetCreatedByNode: sql is " + this.sql;
						this.log.info "userGetCreatedByNode: rowData.size is " + rowData.size;
						this.log.info "userGetCreatedByNode: rowData[0] is " + rowData[0];
						this.log.info "userGetCreatedByNode: createdby is " + createdby; 
					}			
				}
			}		 
		}
	 ...
	 /**
			...
			*/
			...
			"to_char(rawtohex(CREATED_BY_NODE_OID)) CREATED_BY_NODE_OID " +
			"from USER_STATUS_HISTORY where USER_OID = hextoraw('" + oid + "') " +
			"and HISTORY_OID = (Select max(HISTORY_OID) from USER_STATUS_HISTORY " +
			"where USER_OID = hextoraw('" + oid + "')) ";
	...
	*/
	def userGetUserOID(String username) {
		...
		this.log.info("Get userOid fro user : '" + username+ "' , using below query");
		this.log.info("Query : '" + this.sql+ "'");
		try
		{	
			def rowData = this.dbConnExecuteQuery(this.sql);
			  this.log.info("Data returned is : '" + rowData + "'");
			if(!rowData.isEmpty())
			{	
				if (rowData.size == 1) 
				{
	...
	*/
	...
	/**
	* Extract all the ACCOUNT_USER.LOGIN_FAILURES counter associated with the USER_OID 
	* supplied. 
	* @version 1.0
	*
	* @param accountoid The ACCOUNT_USER.USER_OID
	*
	* @return int ACCOUNT_USER.LOGIN_FAILURES value
	*
	*/

	def userGetLoginFailures(String useroid) {
		int fcnt = 0;
		def rowData = "";
					
		this.sql = "select LOGIN_FAILURES from ACCOUNT_USER where " +
			"USER_OID = hextoraw('" + useroid + "')"; 
		
				
		try {	
			 rowData = this.dbConnExecuteQuery(this.sql);
					
			if (rowData.size() > 0) {
				fcnt = rowData[0].LOGIN_FAILURES.toInteger();
				this.log.info "userGetLoginFailures: SQL is " + this.sql ;   
				this.log.info "userGetLoginFailures: LOGIN_FAILUES is " + fcnt ;  
			}
		}
		catch(Throwable e)
		{
	...
	*/
	...
	/**
	* Set user's ACCOUNT_USER.COUNTRY to the string supplied 
	* in the country parameter.  Note that value supplied for  
	...
	*
	*/
	def userSetCountry(String oid, String country) { 
		int rc = 0;
		
		try
		{	
			this.sql = "UPDATE ACCOUNT_USER set COUNTRY = '" + country + "' where " +
				"USER_OID = hextoraw('" + oid + "')";
			...
			if(!rc.toString().isEmpty())
			{
				if (this.log) 
				{
					this.log.info "userSetCountry: rc is "  + rc;
					this.log.info "userSetCountry: sql is " + this.sql;
				}
			}
		}
		catch(Throwable e)
	...
	*/
	...
	/**
	* Set user's ACCOUNT_USER.LOGIN_FAILURES to value supplied.  
	* @version 1.0
	*
	* @param oid ACCOUNT_USER.USER_OID 
	* @param value the numeric value to set in LOGIN_FAILURES.
	*
	* @return Number of rows updated.  Should be one. 
	*
	*/
	def userSetLoginFailures(String oid, int value) { 
		int rc = 0;
		
		this.sql = "UPDATE ACCOUNT_USER set LOGIN_FAILURES = " + value + " where USER_OID = hextoraw('" + oid + "')";	
		...
		{	
			...
			if(!rc.toString().isEmpty())
			{
				if (this.log) 
				{
					this.log.info "userSetLoginFailures: rc is "  + rc;
					this.log.info "userSetLoginFailures: sql is " + this.sql;
				}
			}
		}
		catch(Throwable e)
	...
	*/
	def userChangeUsername(String oid, String username) { 
		...
		this.sql = "UPDATE ACCOUNT_USER set USERNAME = '" + username + "' where USER_OID = hextoraw('" + oid + "')";	
	...
	/**
	* Set user's ACCOUNT_USER.CREATED_BY_NODE_OID value. 
	* @version 1.0
	*
	* @param oid ACCOUNT_USER.USER_OID 
	* @param nodeoid ACCOUNT_USER.CREATED_BY_NODE_OID value to set. 
	*
	* @return Number of rows updated.  Should be one. 
	*
	*/
	def userSetCreatedByNode(String oid, String nodeoid) { 
		int rc = 0;

		this.sql = "UPDATE ACCOUNT_USER set CREATED_BY_NODE_OID = '" + nodeoid + "' where USER_OID = hextoraw('" + oid + "')";	
		...
		{	
			...
			if(!rc.toString().isEmpty())
			{
				if (this.log) 
				{
					this.log.info "userSetCreatedByNode: rc is "  + rc;
					this.log.info "userSetCreatedByNode: sql is " + this.sql;
				}
			}
		}
		catch(Throwable e)
	...
	*/
	def userSetPassword(String oid, String passwd) { 
		...
		this.sql = "UPDATE ACCOUNT_USER set PASSWORD = '" + passwd + "' where USER_OID = hextoraw('" + oid + "')";	
	...
	*/
	def userSetPrivilege(String username, String privilege) { 
		...
		this.sql = "UPDATE ACCOUNT_USER set PRIVILEGE = '" + privilege + "' where USERNAME = hextoraw('" + username + "')";	
