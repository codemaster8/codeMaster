...
public class dbUtils
{
	...
	{
		this.log = log;
		this.log.info("Inside constructor dbUtils(log,envIdentifier)");
		com.eviware.soapui.support.GroovyUtils.registerJdbcDriver("oracle.jdbc.driver.OracleDriver");	
		this.log = log;
		this.dbIdentifier = envIdentifier;          		
		dbConnEstablish(this.dbIdentifier);
		this.log.info("Out constructor dbUtils(log,envIdentifier)");
	...
	{
		this.log.info("Inside ReadDBCrecentials method");
		this.log.info("Arguments passed to this method, envIdentifier : '" +envIdentifier + "'");
		...
		this.log.info("Out ReadDBCrecentials method");
		this.log.info("Return dbUsername : '" + dbUsername + "'");
		this.log.info("Return dbPassword : '" + dbPassword + "'");
	...
	{			
		...
		this.log.info("Argument passed to this method, envIdentifier : '" + envIdentifier + "'");
		...
		else if (envIdentifier == "LP3ST")
		{
			try 
			{
				...
				this.dbConn = Sql.newInstance(db.url, db.user, db.password, db.driver);
				this.log.info("Connected to DB, PSB details : ");
				this.log.info("Url : '" + db.url+ "'");
				this.log.info("Password : '" + db.password+ "'");
			}
			catch(Throwable e) 
			{
				if(this.log) 
		...
		else if(envIdentifier == "DEV")
		{
			log.info "Connecting to DEV DB!!";
			try 
			{
				db = [url: 'jdbc:oracle:thin:@10.31.153.15:2115/decdev1.neustar.com', user: dbUsername, password: dbPassword, driver: 'oracle.jdbc.driver.OracleDriver'];
				this.dbConn = Sql.newInstance(db.url, db.user, db.password, db.driver);
				log.info "dbUserName:"+dbUsername;
				log.info "dbPassword:"+dbPassword;
				log.info "dbDriver:"+"oracle.jdbc.driver.OracleDriver";
				log.info("Connect to DEV DB");
			}
			catch(Throwable e) 
			{
				if(this.log) 
