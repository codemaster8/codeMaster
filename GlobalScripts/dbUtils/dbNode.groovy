...
class dbNode extends dbUtils 
{
	...
	*/
	private dbSetStatusSQL(String nodeId, String status) {
		if (nodeId =~ /urn:dece:org:org:dece:/) {
			...
			this.sql = "update NODE set NODE_STATUS = '%%STATUS%%' where NODE_OID = '%%NODEOID%%'";
			this.sql = sql.replaceAll("%%STATUS%%", status);
			this.sql = sql.replaceAll("%%NODEOID%%", nodeId);
		}
	...
	*/
	def nodeGetNodeCredential(String oid) {
		...
		"X509_NOT_AFTER, " +
		"CREATED_DATE " +
		"from NODE_CREDENTIAL where NODE_OID  = '" + oid + "'";
	...
	*/
	def nodeGetRole(String oid) {
		...
		this.sql = "SELECT ROLE FROM NODE_ROLE WHERE NODE_OID = '" + oid + "'";
	...
	*/
	def nodeGetbyNodeAccountId(String oid) {
		...
		*/
		...
		this.sql = "select to_char(rawtohex(node_oid)) NODE_OID from NODE_ACCOUNT where NODE_ACCOUNT_ID='" + oid + "'";
	...
	*/
	def nodeGetStatus(String oid) {
		...
		*/
		...
		this.sql = "SELECT NODE_STATUS FROM NODE WHERE NODE_OID = '" + oid + "'";
	...
	*/
	def nodeGetCredentialNotAfter(String oid) {
		...
		this.sql = "SELECT X509_NOT_AFTER FROM NODE_CREDENTIAL WHERE NODE_OID = '" + oid + "'";
	...
	*/
	def nodeGetCredentialNotBefore(String oid) {
		...
		this.sql = "SELECT X509_NOT_BEFORE FROM NODE_CREDENTIAL WHERE NODE_OID = '" + oid + "'";
	...
	*/
	def nodeSetRole(String oid, String role) { 
		...
		this.sql = "UPDATE NODE_ROLE SET ROLE = '" + role + "' where NODE_OID = '" + oid + "'";
	...
	*/
	def nodeSetNodeAccountId(String oid, String NodeAccountId) { 
		...
		this.sql = "update NODE_ACCOUNT set NODE_OID='" + oid + "'where NODE_ACCOUNT_ID ='" + NodeAccountId + "'";
	...
	*/
	...
	/**
	* Returns the NODE_OID from NODE table from NODE_ID  
	*/
	def nodeoidGetFromNodeId(String oid) { 
		
	
	String rData = "";

		this.sql = "SELECT to_char(rawtohex(NODE_OID)) NODE_OID FROM NODE where NODE_ID = 'urn:dece:org:org:dece:" + oid + "'";
		...
		try {
			...
			{	
				if (rowData.size == 1) 
				{
					...
					if (this.log) 
					{
						this.log.info "nodeoidGetFromNodeId: sql is " + this.sql;
						this.log.info "nodeoidGetFromNodeId: rowData.size is " + rowData.size;
						this.log.info "nodeoidGetFromNodeId: rowData[0] is " + rowData[0];
					}			
				} 
			} 
		}
